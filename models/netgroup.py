...
class NetGroup(nn.Module):
    ...
    def init_net(self, net_arch):
        local_model_path = r'E:\ChineseJM\pre\pretrained_bert'
        if net_arch == 'bert-base-chinese':
            net = AutoModelForSequenceClassification.from_pretrained(local_model_path, num_labels=self.n_classes)
        elif net_arch == 'bert-base-uncased-2':
            net = model.TextClassifier(num_labels=self.n_classes)
        net.to(self.device)
        return net

    # initialize one optimizer
    def init_optimizer(self, net, lr, lr_linear):
        if self.net_arch == 'bert-base-chinese':
