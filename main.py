...
def oneRun(log_dir, output_dir_experiment, **params):
    ...
    max_epoch = 3 if 'max_epoch' not in params else params['max_epoch']  # 100, 200
    max_step = 3 if 'max_step' not in params else params['max_step']  # 100000, 200000
    ...
    train_labeled_loader, train_unlabeled_loader, dev_loader, test_loader, n_classes = get_dataloader(data_path,
                                                                                                      n_labeled_per_class,
                                                                                                      bs, load_mode)
    ...
    accuracy_classwise = Accuracy(task="multiclass", num_classes=n_classes, average='none')
    ...
    for epoch in range(max_epoch):
        ...
        for batch_label in train_labeled_loader:
            ...
            if step % val_interval == 0:
                ...
                print(
                    '>>Epoch %d Step %d acc_val %f f1_val %f acc_train %f f1_train %f psl_cor %d psl_totl %d pslt_global %f' %
                ...
                print('psl_acc: ', round((psl_correct_eval / psl_total_eval), 3),
                      end=' ') if psl_total_eval > 0 else print('psl_acc: None', end=' ')
            ...
            if load_mode == 'semi':
                for _ in range(ul_ratio):
                    ...
                    try:
                        ...
                        print(f"batch_unlabel device1:{batch_unlabel.device}")
                        #batch_unlabel = {k: v.to(device) for k, v in batch_unlabel.items()}
                    except:
                    ...
                    for i in range(num_nets):
                        ...
                        if labeling_mode == 'soft':
                            ...
                            pseudo_labels_nets.append(F.one_hot(max_idx, num_classes=n_classes))
                    ...
                    for i in range(num_nets):
                        ...
                        if weight_disagreement == 'True':
                            ...
                            disagree_mask = torch.logical_xor(u_psl_masks_nets[(i) % num_nets],
                                                              u_psl_masks_nets[(i + 1) % num_nets])
                            agree_mask = torch.logical_and(u_psl_masks_nets[(i) % num_nets],
                                                           u_psl_masks_nets[(i + 1) % num_nets])
                            disagree_weight_masked = disagree_weight * disagree_mask + (
                                        1 - disagree_weight) * agree_mask
                        ...
                        unsup_loss = consistency_loss(outs_x_ulb_s_nets[i], pseudo_label, loss_type='ce',
                                                      mask=u_psl_mask,
                                                      disagree_weight_masked=disagree_weight_masked)  # loss_type: 'ce' or 'mse'
                    ...
                    print(f"u_psl_mask device2:{u_psl_mask.device}")
                    batch_unlabel['label'] = batch_unlabel['label'].to(device)  # assume 'device' is a GPU device
                    # u_psl_mask = u_psl_mask.to(device) # you can add this line to make sure 'u_psl_mask' is also on GPU
                    gt_labels_u = batch_unlabel['label'][u_psl_mask].to(device)
                    print(f"gt_labels_u device:{gt_labels_u.device}")
                    ...
                    cw_psl_correct = torch.bincount(u_label_psl_hard[u_label_psl_hard == gt_labels_u],
                                                    minlength=n_classes).to('cpu')
